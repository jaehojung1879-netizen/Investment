...
| **Regional alpha model** | `regional_alpha_features.py`, `regional_alpha_model.py` | Does a region-specific ML re-ranking (Ridge/HGBR, annual walk-forward) beat the linear 4-factor blend, using only PIT-reconstructible features? | The 31-feature research-only matrix (§1 of the data map) | **CORRECTED (v2): executed 2026-09-23**, run twice for determinism (workflow run [35826122755](https://github.com/jaehojung1879-netizen/Investment/actions/runs/35826122755), verified from the job log directly). Frozen manifest 381,899 rows, hash `6261031bf59e725ca58342e4f320a8ed62e5d4e797dc6f580c2d372a830543b7`; US training rows 44,456 (2016) → 267,171 (2026); KR training rows 15,294 (2016) → 75,743 (2026); both runs produced the byte-identical `{"US": "NO_MODEL_EVIDENCE", "KR": "NO_MODEL_EVIDENCE"}`. The full report artifact (`regional-alpha-model-v1-discovery-only`, 179MB) was uploaded as a CI artifact but never committed to `docs/results/` — that omission, not non-execution, is why the original version of this table read it as unexecuted. | **EXECUTED — NO_MODEL_EVIDENCE (US and KR)** | **HISTORICAL_DISCOVERY_CLOSED_ON_EXISTING_FEATURE_SET** — closed specifically for the 31-feature price/trend/risk + fundamental-level + fundamental-change matrix; consumed each region's one-shot historical-discovery budget for that feature set. Not "no alpha in public information generally" — see the errata doc. Do not re-run; a genuinely new information axis (investor flow, ownership events, accounting quality, macro interaction) would be a different study, not a retry of this one. |
...
**CORRECTED (v2):** the paragraph above, as originally written, claimed
`regional-alpha-model-v1` had no published result and needed to be run. That
was wrong — see the corrected table row above and
`docs/alpha-research-foundation-v2-errata.md`. It IS a concluded study:
`EXECUTED`, `NO_MODEL_EVIDENCE` both regions,
`HISTORICAL_DISCOVERY_CLOSED_ON_EXISTING_FEATURE_SET`. It must not be
run again.
