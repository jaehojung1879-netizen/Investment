...
3. 중첩 기간과 같은 날의 횡단면을 감안해 유효표본 수를 보수적으로 줄입니다.
4. 지역별 거래비용을 차감하고 표본이 적을수록 0으로 강하게 수축합니다.
5. 최근 성과가 장기 성과보다 악화됐다면 추가 haircut을 적용합니다.

최소 표본과 독립 날짜 수를 충족하지 못하면 기대수익을 0으로 두고 `SHADOW_INSUFFICIENT_HISTORY`로 표시합니다. 데이터 부족을 중립이나 강세로 포장하지 않습니다.

공분산은 선택 종목의 공통 일간 수익률 구간에서 504영업일 lookback, 최소 252영업일 기준으로 Ledoit–Wolf shrinkage를 사용합니다. 계산 실패 시 임의 상관계수나 단위행렬을 넣지 않고 기존 역하방변동성 포트폴리오로 fallback합니다. KR·US 수익률은 현재 로컬통화 기준이므로 환헤지·환율위험을 완전히 모델링하지 못한다는 한계가 있습니다.

Full Kelly는 추정오차와 낙폭에 지나치게 민감하므로 사용하지 않습니다. 기본 Kelly fraction은 0.25이고 매크로 국면은 종목 alpha가 아니라 이 위험예산과 현금 하한만 조정합니다. 최종 비중은 기존 역하방변동성 75%와 제약형 Kelly 25%를 혼합한 뒤 아래 제약을 다시 검증합니다.
