...
**`PARTIALLY_REPAIRED`** — the exact value `pipeline/kr_termination_inventory.foundation_status()` computes from this pass's own completeness matrix, and the value actually written to `docs/results/kr-terminal-action-reconstruction-v2.json`'s `foundationStatus` field. **Not `READY_FOR_V4_PREREGISTRATION`.**

A prior revision of this section labelled the result `READY_FOR_V4_PREREGISTRATION_WITH_PREDECLARED_MISSINGNESS`. That string is not one of `kr_termination_inventory.FOUNDATION_STATUSES`, is not computed anywhere in this repository's code, and directly contradicted this same document's own "What is and is not established" section below (full reconstruction 0/22, `TERMINAL_ACTION_CHAIN` 0/22). It has been corrected here rather than left to stand beside the artifact it disagreed with — exactly the "a status is never forced, and a fabricated-sounding label is worse than an accurate one" discipline this repair line's own module docstring states (`foundation_status`: "Never forced to `READY_FOR_V4_PREREGISTRATION`"). Renaming a `PARTIALLY_REPAIRED` result to something that reads as "ready" without the code, a test, or the artifact backing that name is exactly the failure this repository's promotion-gate invariants (v2.9) and Section 30's own principle ("prefer BLOCKED+explicit-reason over plausible-but-fabricated") exist to prevent.

What this pass legitimately established, and why it does not add up to READY: every realistic evidence-recovery route available in this environment (direct fetch, `WebFetch`, workflow dispatch with real secrets) is confirmed exhausted, not merely untried — recorded above with the exact refusal for each, and this is real, verified progress. But exhausting the routes available to reach evidence is not the same fact as having the evidence: 9 of 22 securities still have terminal consideration/exchange ratio `BLOCKED` (000030, 000830, 002550, 003450, 003600, 010520, 011160, 037620, 079440); successor identity is `BLOCKED` for 4 of them (002550, 003450, 042670, 079440); 2 securities (067250 STX조선해양, 117930 한진해운) are `TERMINATION_TYPE_UNRESOLVED` with no per-share entitlement evidenced anywhere in their collected disclosure history; `TERMINAL_ACTION_CHAIN` is `READY` for 0 of 22; and dividend event-date lineage is `BLOCKED` for all 22. A foundation with a plurality of its fields blocked, by construction (`kr_termination_inventory.foundation_status`'s own routing), cannot read `READY_FOR_V4_PREREGISTRATION` — that value requires every field `READY` or `NOT_APPLICABLE` with zero exceptions, and none of the missing fields above are `NOT_APPLICABLE`.

The genuinely useful output of exhausting the recovery routes is a PRECISE, evidence-backed statement of what a v4 preregistration would need to predeclare a treatment for, before any Alpha outcome is computed against it: the 9-security consideration/ratio gap, the 4-security identity-bridge gap, the 2-security zero-entitlement finding, and the 22-security dividend event-date gap, all listed above with their exact receipt-level basis. That is real, citable groundwork for a future v4 preregistration to state its own bounding treatment against — it is not, itself, a readiness determination, and no future pass on this PR may relabel `PARTIALLY_REPAIRED` as ready without either the missing fields actually resolving or `kr_termination_inventory.py`'s own routing changing, with a test, not a doc edit alone.
