...
const renderValidationLab = (d) => {
  ...
  const alphaPanel = `<div class="vl-card"><div class="vl-h">Alpha diagnostics</div>${[['US',us126],['KR',kr126]].map(([region,row]) => `<details class="vl-fold" open><summary>${region} 126D · Rank IC ${fmt((row.rankIC||{}).mean,'',3)} · monotonicity ${(row.monotonicity||{}).status||'—'}</summary><table class="vl-table"><thead><tr><th>alpha bucket</th><th>raw N</th><th>날짜 / 유효독립</th><th>평균 초과</th><th>비용차감</th><th>승률</th><th>손익비</th><th>CVaR</th></tr></thead><tbody>${bucketRows(row)}</tbody></table><table class="vl-table"><thead><tr><th>Factor attribution</th><th>관측</th><th>Rank IC</th><th>IC IR</th><th>주장 자격</th></tr></thead><tbody>${factorRows(region)}</tbody></table><div class="vl-chart-grid"><section><b>Rolling Rank IC</b>${trendSvg([{label:'3Y IC',points:row.rolling3YRankIC||[]},{label:'5Y IC',points:row.rolling5YRankIC||[]}],[{value:0,labelKo:'0'}],{width:520,height:190,aria:`${region} rolling Rank IC`})}</section><section><b>Rolling Top5-Bottom50 spread (%p)</b>${trendSvg([{label:'3Y spread',points:(row.rolling3YTopBottomSpread||[]).map((x)=>({...x,value:Number(x.value)*100}))},{label:'5Y spread',points:(row.rolling5YTopBottomSpread||[]).map((x)=>({...x,value:Number(x.value)*100}))}],[{value:0,labelKo:'0'}],{width:520,height:190,aria:`${region} rolling top bottom spread`})}</section></div><p class="muted">raw N은 독립 표본이 아닙니다. 126D forward return은 중첩되므로 고유 신호일·유효 독립일·HAC lag 125를 함께 봅니다.</p></details>`).join('')}</div>`;
  ...
  host.innerHTML = `${scorecard}${trustPanel}${alphaPanel}${portfolioPanel}${forecastPanel}<div class="vl-two">${historicalPanel}${prospectivePanel}</div>${kellyPanel}${comparison}${limitations}`;
};
