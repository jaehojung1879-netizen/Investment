Warning: truncated output (original token count: 40019)
Total output lines: 1591
...
const renderValidationLab = (d) => {
  ...
  const kellyPanel = `<div class="vl-card vl-kelly">
    <div class="vl-h">${term('posterior', 'Kelly 증거 결합')} <span class="tag ${activation.code === 'ACTIVE_VALIDATED' ? 'ok' : ''}">${activation.labelKo || EVIDENCE_STATUS[activation.code] || '검증 근거 없음'}…10019 tokens truncated…ard = (r) => {
