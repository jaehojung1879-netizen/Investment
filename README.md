...
- **벤치마크 대비 초과수익은 모델의 알파가 아닙니다.** 버킷의 초과수익은 `유니버스 캐리 + 알파 스프레드`로 분해됩니다. 앞은 재현 유니버스 전체가 벤치마크 대비 낸 부분(구성·베타·기간 효과)이고, 뒤만 순위가 만든 것입니다. 합성 fixture에서도 이 캐리는 KR +2.4%p, US +6.0%p/126일로 커서, 분해하지 않으면 최상위 버킷 "알파"의 절반에서 전부가 캐리입니다. 그래서 **기대수익으로는 날짜별로 캐리를 뺀 알파 스프레드만** 사용하고, 총 초과수익(level)은 투명성을 위해 나란히 발표합니다.
- **순위 정렬 증거가 없으면 사다리를 발표하지 않습니다.** isotonic 회귀는 어떤 입력에도 단조 결과를 만들기 때문에, 정렬 증거 없이 적용하면 잡음이 깔끔한 사다리로 세탁됩니다. 최상위−최하위 스프레드 또는 날짜별 Rank IC의 HAC t가 `minOrderingTStat`(기본 1.96)를 넘고 유효 날짜가 충분할 때만 isotonic 복원과 과거 prior가 허용되며, 미달이면 `ORDERING_NOT_ESTABLISHED`로 표시하고 Kelly 기대수익에 쓰지 않습니다. 이 1.96은 표에서 가져온 값이 아니라 이 gate 자체를 귀무가설 하에서 시뮬레이션해 실제 기각률이 3.6~5.0%가 되도록 맞춘 값입니다.
