...
- PIT 재무는 `replay-v6`부터 한국 116종목에 들어왔고 미국은 아직 없습니다. FMP가 재무제표 엔드포인트를 무료 티어에서 제외한 것은 실측입니다(402). **SEC의 GitHub Actions IP 거절도 이제 실측입니다(2026-09-04).** 한때 "헤더 때문일 수 있다"고 정정했었는데, 그 정정을 다시 거둡니다 — 가설은 틀렸고 원래 결론이 맞았습니다. 다만 그때는 가정이었고 지금은 증거가 있습니다. `Probe SEC header isolation`이 같은 실행·같은 러너·같은 분에 헤더 세트 4종 × 호스트 2곳 = 8회를 던졌고 **8회 전부 403**, 그것도 **바이트 수까지 동일한**(data.sec.gov 4,819B, www.sec.gov 1,925B) 같은 차단 페이지였습니다. `Accept-Encoding: gzip, deflate`를 추가해도, UA를 SEC 문서 형식으로 바꿔도, 정책이 명시한 최소 세트만 보내도 응답이 한 바이트도 달라지지 않습니다 — SEC가 우리 헤더를 **읽지도 않는다**는 뜻이고, 거절은 요청 모양이 아니라 출발지에서 일어납니다. 판정은 `REFUSED_ON_EVERY_HEADER_SET`입니다.
