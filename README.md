...
- **입력 0.2%가 평가 일정 84%를 흔듭니다.** `block_dates`는 측정 가능한 날짜를 앞에서부터 greedy로 훑어 겹치지 않는 블록을 고르므로, 날짜 하나가 들어오거나 빠지면 그 뒤 블록이 전부 재고정됩니다. 두 실행의 롤링 평가지점 104개 중 **공유는 29개(16.2%)** 였습니다. 120,371행 중 250행 변화가 만든 결과입니다.
- **그래서 `replayDeterminism` 가드가 들어갔고, 어긋나면 보고서를 막습니다.** 원장은 매일 자라야 하므로 불변식은 "절대 안 변한다"가 아니라 **접두 안정성**입니다 — 이미 발표된 블록은 종료일까지 그대로여야 하고, 이미 발표된 날짜의 단면 구성도 그대로여야 하며, 마지막 기록 뒤에 붙는 것만 허용됩니다. 블록의 **종료일**까지 지문에 넣는 이유는 `shared_block_dates`가 두 selector 중 **늦은** 종료일을 쓰기 때문입니다: 진입일이 같아도 종료일이 밀리면 그 뒤 블록이 전부 밀리는데, 진입일만 보는 지문은 그걸 "안정"이라고 부릅니다. 어긋나면 `contractValidation`이 `replay_not_reproducible_past_changed`로 실패하고 `audit_portfolio.py`가 종료코드 1을 냅니다 — **과거가 움직인 위에서 계산된 성적표는 전략에 대한 증거가 아니기 때문입니다.** `replayVersion`이 바뀌면 새 실험이므로 위반이 아닙니다.
