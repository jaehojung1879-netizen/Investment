...
- **`vintageMacro`도 현재 시작일(2013-01-01)에서는 닫히지 않습니다 — 측정 결과입니다(2026-09-04).** 파생 컬럼 결함은 실재했고 고쳤지만, 그것이 유일한 장애물이 아니었습니다. ALFRED 프로브를 실제로 돌려보니 패널 28개 컬럼 중 **최소 8개가 리플레이 시작일까지 빈티지가 닿지 않습니다**: `HY_Spread`·`IG_Spread`는 첫 빈티지가 **2023-09-05**(ICE BofA 계열이 그때 ALFRED에 들어옴, 그 전 795개 기간에 대한 빈티지가 아예 없음), `Broad_Dollar` 2019-02-04, `Core_PPI` 2015-03-13, `USD_KRW` 2014-03-18, `Headline_PPI` 2014-02-19, `Korea_10Y`·`Korea_3M` 2013-06-03입니다. 이들을 켜면 그 컬럼은 초기 리플레이 날짜에서 통째로 비고 판정만 `PIT_EXACT`로 올라갑니다 — 그래서 켜지 않습니다. 즉 게이트 두 개 다 현재 정의·현재 시작일에서는 도달 불가능하며, `promotionEligible`은 계속 false입니다.
- **그 결손은 시작일을 옮기면 대부분 사라지지만, 그것은 별개의 결정입니다.** 2018-01-01 기준으로는 위 8개 중 6개가 서빙됩니다(`HY_Spread`·`IG_Spread`는 2023년이라 여전히 비어 있음). 5년치 증거를 라벨과 맞바꾸는 거래이므로 여기서 임의로 하지 않습니다 — DART가 2015년부터라 시작일을 옮기지 않는다고 적어둔 것과 같은 이유입니다.
- **첫 실행에서 6개 시리즈는 프로브 자신의 버그로 못 쟀습니다.** `fredapi`의 `get_series_all_releases`는 전체 real-time 구간을 한 번에 요청하는데, 일별 시리즈는 ALFRED의 **빈티지 날짜 2,000개 상한**에 걸립니다(`DGS10` 5,103개). `NFCI`·`ANFCI`는 정확히 100,000행이 와서 행 상한에 잘렸습니다. 그래서 프로브는 `series/vintagedates`(첫 빈티지 날짜)와 `realtime_start=realtime_end=T`로 고정한 스냅샷(그날 그 컬럼이 실제로 서빙한 것)만 쓰도록 바꿨습니다 — 둘 다 상한에 걸리지 않습니다. 실패는 `"the ALFRED call did not return"`이 아니라 벤더가 말한 문장 그대로 기록합니다.
