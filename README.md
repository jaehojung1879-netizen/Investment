...
- PIT 재무는 `replay-v6`부터 한국 116종목에 들어왔고 미국은 아직 없습니다. FMP가 재무제표 엔드포인트를 무료 티어에서 제외한 것은 실측입니다(402). **SEC 쪽 "Actions IP 사이트 차원 차단"은 아직 확정된 결론이 아니며, 그렇게 적었던 것을 정정합니다** — 두 SEC 프로브 모두 SEC 접근 정책이 요구하는 `Accept-Encoding: gzip, deflate`를 보내지 않았고, 방증으로 든 `institutional_13f`도 같은 헤더 세트를 씁니다(독립적인 관측이 아니었습니다). UA도 정책이 문서화한 `회사명 연락처`가 아니라 `앱이름/1.1 연락처`였습니다. `"Undeclared Automated Tool"`은 헤더 세트가 정책에 안 맞을 때 나오는 페이지이자 IP 거절이 밖에서 보이는 모습이기도 한데, 둘을 분리하지 않았습니다. `Probe SEC header isolation`이 같은 실행·같은 러너에서 출하 중인 헤더를 기준선으로 두고 조합만 바꿔 분리합니다. 미국 절반이 여전히 가격 슬리브뿐이므로 전체 결과의 근거는 계속 `PRICE_SLEEVES_ONLY_AUDIT_PROXY`이며, 한국 단면에 한해 4-factor로 매겨집니다. 지역을 합친 헤드라인을 4-factor 실전 모델의 동일조건 검증으로 읽으면 안 됩니다.
