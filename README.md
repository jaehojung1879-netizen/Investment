...
- **다만 SEC는 2026-08-15에 우리에게 응답했습니다.** `data/institutional_13f_cache.json`이 `sourceMode: LIVE_SEC`, `fetchedAt: 2026-08-15T23:00:24Z`이고, 출처는 지금 전부 거절당하는 바로 그 `data.sec.gov`입니다. 즉 이건 이 프로젝트에 대한 상시 정책이 아니라 **그 사이에 뭔가 바뀐 것**입니다. 그리고 헤더 분리 실행은 러너를 고정한 채로 했으므로 **송신 IP도 같이 고정**돼 있었습니다 — 한 주소가 여덟 번 거절당한 건 관측 하나를 여덟 번 반복한 것입니다. `Probe SEC egress addresses`가 병렬 러너로 팬아웃해 그 축을 흔듭니다. 일부 주소만 서빙되면 "Actions 풀 차단"은 틀린 설명이고 러너를 갈아가며 재시도하는 백필이 성립합니다. 서로 다른 주소가 2개 미만이면 판정을 거부합니다 — 변수를 안 흔든 실행이기 때문입니다.
- **그 실험이 전부 거절로 끝나면, 그때 미국 PIT 재무는 SEC 말고 다른 벤더가 필요하고 그건 키가 필요합니다.** 남은 무료 후보는 Polygon.io와 SimFin이고 둘 다 발급이 필요합니다. SEC 자체는 키 제도가 없으므로 이 경로는 출발지가 바뀌지 않는 한 닫혀 있습니다.
