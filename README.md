...
- PIT 재무는 `replay-v6`부터 한국 116종목에 들어왔고 미국은 아직 없습니다. FMP는 **아직 결론이 아닙니다 — 402를 받고 본문을 버렸습니다.** 프로브가 응답 본문을 읽어놓고 `except ValueError`에서 `"non-JSON response"`로 덮어써 버려서, FMP가 어느 플랜을 요구하는지 적어 보낸 문장이 사라졌습니다. "무료 티어에서 뺐다"는 그 빈 402에서 제가 추론한 것입니다. 이제 본문을 그대로 남기고, ① 키가 다른 엔드포인트에서는 사는지(`profile`·`quote`) ② `/stable`과 `/api/v3` 양쪽 URL 형태 를 각각 확인해서 **죽은 키 / 플랜 제한 / 경로 문제**를 갈라냅니다. **SEC의 GitHub Actions IP 거절도 이제 실측입니다(2026-09-04).** 한때 "헤더 때문일 수 있다"고 정정했었는데, 그 정정을 다시 거둡니다 — 가설은 틀렸고 원래 결론이 맞았습니다. 다만 그때는 가정이었고 지금은 증거가 있습니다. `Probe SEC header isolation`이 같은 실행·같은 러너·같은 분에 헤더 세트 4종 × 호스트 2곳 = 8회를 던졌고 **8회 전부 403**, 그것도 **바이트 수까지 동일한**(data.sec.gov 4,819B, www.sec.gov 1,925B) 같은 차단 페이지였습니다. `Accept-Encoding: gzip, deflate`를 추가해도, UA를 SEC 문서 형식으로 바꿔도, 정책이 명시한 최소 세트만 보내도 응답이 한 바이트도 달라지지 않습니다 — SEC가 우리 헤더를 **읽지도 않는다**는 뜻이고, 거절은 요청 모양이 아니라 출발지에서 일어납니다. 판정은 `REFUSED_ON_EVERY_HEADER_SET`입니다.
...
- **따라서 미국 PIT 재무는 SEC 말고 다른 벤더가 필요하고, 그건 키가 필요합니다.** 남은 무료 후보는 Polygon.io와 SimFin입니다. SEC는 키 제도가 없으므로 GitHub Actions에서 나가는 한 닫혀 있습니다.
