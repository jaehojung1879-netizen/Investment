...
첫 화면은 `데이터 신뢰 상태 → 오늘의 시장 요약 → KR·US 리서치 후보 → 모델 포트폴리오` 네 블록으로 구성합니다. 기술 상태 코드는 상세에서만 보여주고 기본 화면에는 `검증 데이터 축적 중`, `Paper 운용 중`, `안전 차단` 같은 사용자용 상태를 표시합니다. 글로벌 마켓 카드는 현재값·1일 등락과 **3M 추이·3M 수익률**을 분리하며, 상세 차트 기본 기간도 3M입니다.
